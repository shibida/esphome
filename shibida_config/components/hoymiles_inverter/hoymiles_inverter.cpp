...
void HoymilesInverter::loop() {
  ...
  if (check_updated(this->inverter_->DevInfo(), dev_info_last_update_)) {
    ...
    ESP_LOGD(TAG, "loop(): DevInfo updated: %s, %d, %d", this->inverter_->DevInfo()->getHwVersion(),
             this->inverter_->isProducing(), this->inverter_->isReachable());
  }
  if (check_updated(this->inverter_->Statistics(), stat_last_update_)) {
    ...
    for (auto &it : dc_channels) {
      if (this->channels_.size() > i) {
        this->channels_[i++]->updateSensors(this->inverter_->isProducing(), this->inverter_->Statistics(),
                                            ChannelType_t::TYPE_DC, it);
      }
    }
    if ((ac_channels.size() > 0) && (this->ac_channel_ != nullptr)) {
      this->ac_channel_->updateSensors(this->inverter_->isProducing(), this->inverter_->Statistics(),
                                       ChannelType_t::TYPE_AC, ac_channels.front());
    }
    if ((inv_channels.size() > 0) && (this->inverter_channel_ != nullptr)) {
      this->inverter_channel_->updateSensors(this->inverter_->isProducing(), this->inverter_->Statistics(),
                                             ChannelType_t::TYPE_INV, inv_channels.front());
    }
  }
...
void HoymilesPlatform::set_pins(esphome::InternalGPIOPin *sdio, esphome::InternalGPIOPin *clk,
                                esphome::InternalGPIOPin *cs, esphome::InternalGPIOPin *fcs) {
...
void HoymilesPlatform::setup() {
  this->hoymiles_->getRadioCmt()->setDtuSerial(0x199980101901);
  this->hoymiles_->getRadioCmt()->setCountryMode(CountryModeId_t::MODE_EU);
  this->hoymiles_->getRadioCmt()->setInverterTargetFrequency(865e6);
  this->hoymiles_->getRadioCmt()->setPALevel(20);
...
void HoymilesPlatform::loop() { this->hoymiles_->getRadioCmt()->loop(); }
